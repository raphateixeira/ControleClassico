...
$$\mathrm{L}\{f(t)\} = F(s) = \int_0^{\infty} f(t)\,e^{-st}\,dt, \qquad s = \sigma + j\omega \in \mathbb{C}$$

$$\mathrm{L}\left\{\frac{d^n f}{dt^n}\right\} = s^n F(s) \qquad \text{(condições iniciais nulas)}$$

| Propriedade | Expressão |
|:------------|:----------|
| Linearidade | $\mathrm{L}\{\alpha f + \beta g\} = \alpha F(s) + \beta G(s)$ |
| Deslocamento em $s$ | $\mathrm{L}\{e^{-at}f(t)\} = F(s+a)$ |
...
## Exemplo Mínimo: 1ª Ordem

Sistema genérico de 1ª ordem, ganho $K$ e constante de tempo $\tau$:
$$\tau\,\dot{c}(t) + c(t) = K\,r(t)$$
...
::: {.fragment}
$$\boxed{G(s) = \frac{C(s)}{R(s)} = \frac{K}{\tau s+1}}$$
:::
...
Entrada degrau unitário: $r(t) = u(t) \;\Rightarrow\; R(s) = \dfrac{1}{s}$

::: {.fragment}
$$C(s) = G(s)\,R(s) = \frac{K}{s(\tau s+1)}$$
:::

::: {.fragment}
Frações parciais:
$$\frac{K}{s(\tau s+1)} = \frac{A}{s} + \frac{B}{\tau s+1}, \qquad A = K, \quad B = -K\tau$$
$$C(s) = \frac{K}{s} - \frac{K}{s+\dfrac{1}{\tau}}$$
:::

::: {.fragment}
Transformada inversa (par $e^{-at} \leftrightarrow \frac{1}{s+a}$):
$$\boxed{c(t) = K\left(1 - e^{-t/\tau}\right)u(t)}$$
:::

::: {.callout-tip}
Mesmo caminho vale para qualquer sistema de 1ª ordem — tema da próxima aula.
:::
...
LKT (lei das tensões de Kirchhoff): $v(t) = v_R(t) + v_L(t) + v_C(t)$

$$v_R(t) = R\,i(t), \qquad v_L(t) = L\,\dot{i}(t), \qquad i(t) = C\,\dot{v}_C(t)$$

---

## Circuito RLC — EDO e FT

Substituindo $i(t) = C\,\dot v_C(t)$ na LKT:

::: {.fragment}
$$\boxed{LC\,\ddot{v}_C(t) + RC\,\dot{v}_C(t) + v_C(t) = v(t)}$$
:::
...
::: {.callout-tip}
Mesma estrutura do massa-mola-amortecedor: $L\leftrightarrow m$, $R\leftrightarrow b$, $1/C\leftrightarrow k$.
:::
